...
def process_video(video_path, lowscale_dir, upscale_dir, api_url, headers, json_payload_template):
    extract_frames(video_path, lowscale_dir)

    for frame in os.listdir(lowscale_dir):
        frame_path = os.path.join(lowscale_dir, frame)
        upscaled_image = upscale_frame(frame_path, api_url, headers, json_payload_template)

        if upscaled_image:
            with open(os.path.join(upscale_dir, frame), 'wb') as file:
