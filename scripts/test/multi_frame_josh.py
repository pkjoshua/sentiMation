...
def get_frames_from_directory(directory_path):
    frames = []
    for filename in sorted(os.listdir(directory_path)):
        if filename.endswith(('.png', '.jpg', '.jpeg')):  # Add other formats if necessary
...
class Script(scripts.Script):
    def title(self):
        return "Multi-frame Video - V0.72-beta (josh version)"
    ...
    def ui(self, is_img2img):   
        ...
        with gr.Row(variant="panel"):
            frames_directory = gr.Textbox(label="Frames Directory Path", placeholder="Enter path to frames directory", type="text")
        ...
        return [append_interrogation, frames_directory, first_denoise, third_frame_image, color_correction_enabled, unfreeze_seed, render_grid,grid_rows, use_nth_frame, loopback_source]

    def run(self, p, append_interrogation, frames_directory, first_denoise, third_frame_image, color_correction_enabled, unfreeze_seed, render_grid, grid_rows, use_nth_frame, loopback_source):
        freeze_seed = not unfreeze_seed
        loaded_frames = get_frames_from_directory(frames_directory)
        use_nth_frame = int(use_nth_frame)
        grid_rows = int(grid_rows)
        
        loops = math.floor(len(loaded_frames) / use_nth_frame)
        ...
        for n in range(batch_count):
            ...
            for i in range(loops):
                ...
                reference_image = loaded_frames[i*use_nth_frame]
                ...
                if(i > 0):
                    ...
                    if third_frame_image != "None" and i > 1:
                        ...
                        msk.paste(loaded_frames[i-1].resize((initial_width, p.height), Image.ANTIALIAS), (0, 0))
                        msk.paste(p.control_net_input_image, (initial_width, 0))
                        msk.paste(loaded_frames[third_image_index].resize((initial_width, p.height), Image.ANTIALIAS), (initial_width*2, 0))
                    ...
                    processed_frames.append(p.control_net_input_image)
                ...
                loaded_frames.append(processed.images[0])
